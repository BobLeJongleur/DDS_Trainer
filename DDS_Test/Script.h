 ...
 */
typedef uint16_t VariableSizeType;
...
typedef uint16_t CodeSkipSizeType;
#else
...
namespace FunctionCallspace
{
	...
	inline const char* ToString(FunctionCallspace::Type Callspace)
	{
		switch (Callspace)
		{
		case Absorbed:
...
//FArchive& operator<<(FArchive& Ar, EFunctionFlags& Flags)
//{
//	Ar << (uint32_t&)Flags;
//	return Ar;
//}
...
enum EExprToken
{
	...
	EX_TextConst			= 0x29, // SDK::FText constant
...
{
	/* Text is an empty string. The bytecode contains no strings, and you should use SDK::FText::GetEmpty() to initialize the SDK::FText instance. */
	Empty,
	...
	LocalizedText,
	/** Text is culture invariant. The bytecode will contain one string, and you should use SDK::FText::AsCultureInvariant to initialize the SDK::FText instance. */
	InvariantText,
	/** Text is a literal FString. The bytecode will contain one string, and you should use SDK::FText::FromString to initialize the SDK::FText instance. */
	LiteralString,
	/** Text is from a string table. The bytecode will contain an object pointer (not used) and two strings - the table ID, and key - and should be found via SDK::FText::FromStringTable */
	StringTableEntry,
};
...
public:
	...
	FBlueprintExceptionInfo(EBlueprintExceptionType::Type InEventType, const SDK::FText& InDescription)
		: EventType(InEventType)
	...
	const SDK::FText& GetDescription() const
	{
		return Description;
	}
protected:
	EBlueprintExceptionType::Type EventType;
	SDK::FText Description;
};
...
//
//	FScriptInstrumentationSignal(EScriptInstrumentation::Type InEventType, const UObject* InContextObject, UFunction* InFunction, const int32_t LinkId = INDEX_NONE)
//		: EventType(InEventType)
...
//	int32_t GetScriptCodeOffset() const;
//
//	/** Returns the latent link id for latent events */
//	int32_t GetLatentLinkId() const { return LatentLinkId; }
//
//protected:
//
...
//	static void SetScriptMaximumLoopIterations( const int32_t MaximumLoopIterations );
//};
//
